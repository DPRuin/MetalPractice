...
public class MetalView:MTKView, NSWindowDelegate {
    ...
    private func setupTexture() {
        ...
        let textureLoder = MTKTextureLoader(device: device!)
       
        do {
            texture = try textureLoder.newTexture(URL: URL(fileURLWithPath: path!), options: nil)
        } catch let error {
            printView("--textureloder-\(error)")
        }
    }
    ...
    private func render(in view: MTKView) {
        ...
        command_encoder.setBuffer(timerBuffer, offset: 0, index: 0)
        command_encoder.setTexture(texture, index: 1)
